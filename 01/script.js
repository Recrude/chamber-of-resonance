...
// 시트2로 데이터를 보내기 위해 URL 끝에 ?sheet=2 파라미터 추가 필요
const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbz6V7_OMaYpLlC7SufZghsqwkWcuU2SD33RPvFolgnSHUshmxeHloBqeAONrcZp19zP/exec';

// 신청 오픈 시간 설정 (2026년 1월 16일 19:00)
const OPEN_TIME = new Date('2026-01-16T19:00:00');
...
function checkOpenTime() {
    ...
    if (now < OPEN_TIME) {
        // 아직 오픈 시간 전이면 오버레이 표시
        waitingOverlay.style.display = 'flex';
        
        // 남은 시간 계산하여 정확한 시간에 오버레이 제거
        const timeUntilOpen = OPEN_TIME - now;
        setTimeout(() => {
            waitingOverlay.style.display = 'none';
        }, timeUntilOpen);
    } else {
        // 이미 오픈 시간이 지났으면 오버레이 숨김
        waitingOverlay.style.display = 'none';
    }
}
...
participationForm.addEventListener('submit', async (e) => {
    ...
    if (!formData.name || !formData.email || !formData.contact) {
        submitBtn.textContent = '필수 항목을 입력해주세요';
